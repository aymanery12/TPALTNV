    ...
    <div class="min-h-screen bg-[#0f0f1a] text-white flex flex-col">
      ...
      <main class="flex-1 max-w-[1200px] mx-auto w-full p-4 md:p-8">
        ...
        <div *ngIf="!isLoading && !book" class="text-center py-32 text-slate-400">
          ...
          <p class="text-xl font-bold">Livre introuvable</p>
          <a routerLink="/catalog" class="mt-4 inline-block text-amber-400 hover:underline">← Retour au catalogue</a>
        </div>
        ...
        <div *ngIf="!isLoading && book">
          ...
          <nav class="flex items-center gap-2 text-sm text-slate-400 mb-6">
            <a routerLink="/home" class="hover:text-white transition-colors">Accueil</a>
            <span>›</span>
            <a routerLink="/catalog" class="hover:text-white transition-colors">Catalogue</a>
            <span>›</span>
          ...
          <div class="flex flex-col md:flex-row gap-10 mb-12">
            ...
            <div class="flex-1">
              ...
              <p class="text-slate-400 mb-4">
                par <span class="text-white font-medium">
                  {{ book.author?.join(', ') || 'Auteur inconnu' }}
                </span>
              </p>
          ...
          <div class="bg-gradient-to-r from-amber-500/10 to-orange-500/10 border border-amber-500/20 rounded-2xl p-6 mb-10">
            <div class="flex items-center gap-2 mb-3">
              ...
              <h2 class="font-bold text-amber-400">Résumé par Gemini AI</h2>
            ...
            <div *ngIf="!isLoggedIn" class="text-slate-400 text-sm">
              <a routerLink="/login" class="text-amber-400 underline">Connectez-vous</a> pour obtenir un résumé IA.
            </div>
            <div *ngIf="aiLoading" class="flex items-center gap-2 text-slate-400 text-sm">
              <span class="material-symbols-outlined animate-spin text-sm">progress_activity</span>
              Gemini génère un résumé...
            </div>
            ...
            <p *ngIf="!aiSummary && !aiLoading && isLoggedIn" class="text-slate-500 text-sm">
              Cliquez sur "Générer" pour obtenir un résumé intelligent de ce livre.
            </p>
          </div>

          <!-- Reviews -->
          <div class="mb-10">
            <h2 class="text-xl font-bold mb-6">Avis clients ({{ reviews.length }})</h2>

            <!-- Add review -->
            <div *ngIf="isLoggedIn" class="bg-white/5 border border-white/10 rounded-2xl p-6 mb-6">
              <h3 class="font-semibold mb-4">Donner mon avis</h3>
            ...
            <div *ngIf="!isLoggedIn" class="text-slate-400 text-sm mb-6">
              <a routerLink="/login" class="text-amber-400 underline">Connectez-vous</a> pour laisser un avis.
            </div>
            ...
            <div *ngFor="let r of reviews"
              ...
              <div class="flex items-center justify-between mb-2">
                <div class="flex items-center gap-2">
                  ...
                  <span class="font-semibold text-sm">{{ r.user?.username || 'Anonyme' }}</span>
                </div>
                <div class="flex gap-0.5">
                  <span *ngFor="let i of [1,2,3,4,5]"
...
export class BookDetailsComponent implements OnInit {
  ...
  loadAiSummary(): void {
    ...
    this.chatService.getBookSummary(this.book.id).subscribe({
      ...
      error: () => { this.aiSummary = 'Impossible de générer le résumé.'; this.aiLoading = false; }
    });
  }
  ...
  addToCart(): void {
    ...
    this.showToast(`"${this.book.title}" ajouté au panier !`);
  }
  ...
  toggleWishlist(): void {
    ...
    this.showToast(this.isInWishlist ? 'Ajouté aux favoris !' : 'Retiré des favoris');
  }
  ...
  submitReview(): void {
    ...
    this.bookService.addReview(this.book.id, {
      ...
      error: () => { this.reviewLoading = false; this.showToast('Erreur lors de la publication.'); }
    });
  }
